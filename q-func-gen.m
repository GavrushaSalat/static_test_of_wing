...
q(1:n_points) = a_linear + b_linear * x_points(1:n_points);

%% 2. Параболический закон (следующие 3 точки)
a_parab = rand() * 1;
b_parab = rand() * 2;
c_parab = rand() * 3;
q(1:n_points) = a_parab + b_parab * x_points(1:n_points) + c_parab * x_points(1:n_points).^2;
...
q(1:n_points) = A_hyper ./ (x_points(1:n_points) + C_hyper) + B_hyper;

% Нормализуем, чтобы значения были в разумном диапазоне
q = q / max(q) * 5; % масштабируем до максимума 5

%% Визуализация
figure('Position', [100, 100, 800, 600]);

subplot(2,1,1);
plot(x_points, q, 'ro-', 'LineWidth', 2, 'MarkerSize', 8, 'MarkerFaceColor', 'r');
xlabel('Координата x');
ylabel('Нагрузка q(x)');
...
% Подписи точек
for i = 1:n_points
    text(x_points(i), q(i)+0.1, sprintf('q_%d=%.2f', i, q(i)), ...
...
text(0.1, 0.5, sprintf('Диапазон значений: [%.2f, %.2f]', min(q), max(q)), 'FontSize', 10);
